...
public static class Utils
{
    ...
    /// <summary>
    /// Starts the specified service asynchronously.
    /// </summary>
    /// <param name="svcName">
    ...
    /// </returns>
    public static async Task<bool> StartServiceAsync(string svcName, Action<int> progressCallback = null)
    {
        try
        {
            ServiceController svc = new(svcName);
            switch (svc.Status)
            {
                ...
                case ServiceControllerStatus.StopPending:
                    if (!await svc.WaitForStatusAsync(ServiceControllerStatus.Stopped, progressCallback))
                    {
                        return false;
                    }
                    break;
                default:
                    // service is already running, is paused, or is pending pause/continue
                    return true;
            }

            return await svc.WaitForStatusAsync(ServiceControllerStatus.Running, progressCallback);
        }
        catch
        {
            return false;
    ...
    /// <summary>
    /// Stops the specified service asynchronously.
    /// </summary>
    /// <param name="svcName">
    /// The service name, as shown in <c>services.msc</c>
    /// (NOT to be confused with its display name).
    ...
    /// </returns>
    public static async Task<bool> StopServiceAsync(string svcName, Action<int> progressCallback = null)
    {
        try
        {
            ServiceController svc = new(svcName);
            switch (svc.Status)
            {
                ...
                case ServiceControllerStatus.PausePending:
                    if (!await svc.WaitForStatusAsync(ServiceControllerStatus.Paused, progressCallback))
                    {
                        return false;
                    }
                    break;
                ...
                case ServiceControllerStatus.ContinuePending:
                    if (!await svc.WaitForStatusAsync(ServiceControllerStatus.Running, progressCallback))
                    {
                        return false;
                    }
                    break;
            ...
            return await svc.WaitForStatusAsync(ServiceControllerStatus.Stopped, progressCallback);
        }
        catch
        {
            return false;
    ...
    private static async Task<bool> WaitForStatusAsync(this ServiceController svc, ServiceControllerStatus status, Action<int> progressCallback = null)
    {
