...
{
    private readonly OverlayConf Config;
    ...
    // prevent overlay from being focused part 2: electric boogaloo
    ...
    public MainForm(bool autoRun)
    {
        ...
        try
        {
            Config = ObcConfig.Load(Paths.Config).Overlays;
        }
        catch (Exception ex)
        {
            // prevent crash if config is missing
            if (ex is InvalidConfigException or InvalidOperationException or
                FileNotFoundException or DirectoryNotFoundException)
            {
                Config = new OverlayConf();
            }
            else
            {
                throw;
    ...
    {
        bool blurEnable = Config.BlurMode == BlurMode.Automatic
            ? GetBlurEnabled()
            : Config.BlurMode == BlurMode.Enabled;
